...
def saveckpt(model, path='model'):
    dic = dict(width=model.width, grid=model.grid, k=model.k, mult_arity=
        model.mult_arity, base_fun_name=model.base_fun_name,
        ...
        sb_trainable, device=model.place, state_id=model.state_id,
        auto_save=model.auto_save, ckpt_path=model.ckpt_path)
...
def loadckpt(path='model'):
    ...
    model_load = MultKAN(width=config['width'], grid=config['grid'], k=
        ...
        'grid_eps'], grid_range=config['grid_range'], sp_trainable=config[
        'sp_trainable'], sb_trainable=config['sb_trainable'], device=config
        ['device'], state_id=config['state_id'], auto_save=config[
        'auto_save'], first_init=False, ckpt_path=config['ckpt_path'])
    ...
    for l in range(depth):
        ...
        for j in range(out_dim):
            for i in range(in_dim):
                ...
                model_load.symbolic_fun[l].funs[j][i] = SYMBOLIC_LIB[fun_name][
                    0]
