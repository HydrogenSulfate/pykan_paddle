...
def runner1(width, dataset, grids=[5, 10, 20], steps=20, lamb=0.001,
    prune_round=3, refine_round=3, edge_th=0.01, node_th=0.01, metrics=None,
    seed=1):
    result = {}
    result['test_loss'] = []
    result['c'] = []
    ...
    def collect(evaluation):
        ...
        if metrics != None:
            for i in range(len(metrics)):
                result[metrics[i].__name__].append(metrics[i](model,
                    dataset).item())
...
def pareto_frontier(x, y):
    pf_id = np.where(np.sum((x[:, None] <= x[None, :]) * (y[:, None] <= y[
        None, :]), axis=0) == 1)[0]
    x_pf = x[pf_id]
    y_pf = y[pf_id]
