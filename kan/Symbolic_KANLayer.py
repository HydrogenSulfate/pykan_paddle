...
class Symbolic_KANLayer(paddle.nn.Layer):
    """
    KANLayer class
    ...
    def __init__(self, in_dim=3, out_dim=2, device='cpu'):
        ...
        self.in_dim = in_dim
        out_1 = paddle.base.framework.EagerParamBase.from_tensor(tensor=
            paddle.zeros(shape=[out_dim, in_dim]))
        out_1.stop_gradient = not False
        self.mask = out_1
        self.funs = [[(lambda x: x * 0.0) for i in range(self.in_dim)] for
            j in range(self.out_dim)]
        self.funs_avoid_singularity = [[(lambda x, y_th: ((), x * 0.0)) for
            i in range(self.in_dim)] for j in range(self.out_dim)]
        self.funs_name = [['0' for i in range(self.in_dim)] for j in range(
            self.out_dim)]
        self.funs_sympy = [[(lambda x: x * 0.0) for i in range(self.in_dim)
            ] for j in range(self.out_dim)]
        self.affine = paddle.base.framework.EagerParamBase.from_tensor(tensor
            =paddle.zeros(shape=[out_dim, in_dim, 4]))
        self.device = device
        ...
        for i in range(self.in_dim):
            ...
            for j in range(self.out_dim):
                if singularity_avoiding:
                    xij = self.affine[j, i, 2] * self.funs_avoid_singularity[j
                        ][i](self.affine[j, i, 0] * x[:, [i]] + self.affine
                        [j, i, 1], paddle.to_tensor(data=y_th))[1
                        ] + self.affine[j, i, 3]
                else:
                    xij = self.affine[j, i, 2] * self.funs[j][i](self.
                        affine[j, i, 0] * x[:, [i]] + self.affine[j, i, 1]
                        ) + self.affine[j, i, 3]
                postacts_.append(self.mask[j][i] * xij)
            postacts.append(paddle.stack(x=postacts_))
        postacts = paddle.stack(x=postacts)
        postacts = postacts.transpose(perm=[2, 1, 0, 3])[:, :, :, 0]
        y = paddle.sum(x=postacts, axis=2)
        return y, postacts
    ...
    def get_subset(self, in_id, out_id):
        ...
        sbb.mask.data = self.mask.detach()[out_id][:, in_id]
        sbb.funs = [[self.funs[j][i] for i in in_id] for j in out_id]
        sbb.funs_avoid_singularity = [[self.funs_avoid_singularity[j][i] for
            i in in_id] for j in out_id]
        sbb.funs_sympy = [[self.funs_sympy[j][i] for i in in_id] for j in
            out_id]
        sbb.funs_name = [[self.funs_name[j][i] for i in in_id] for j in out_id]
        sbb.affine.data = self.affine.detach()[out_id][:, in_id]
        return sbb

    def fix_symbolic(self, i, j, fun_name, x=None, y=None, random=False,
        a_range=(-10, 10), b_range=(-10, 10), verbose=True):
        """
        fix an activation function to be symbolic
        ...
        if isinstance(fun_name, str):
            ...
            if x == None or y == None:
                ...
                if random == False:
                    self.affine.detach()[j][i] = paddle.to_tensor(data=[1.0,
                        0.0, 1.0, 0.0])
                else:
                    self.affine.detach()[j][i] = paddle.rand(shape=[4]) * 2 - 1
                return None
            else:
                params, r2 = fit_params(x, y, fun, a_range=a_range, b_range
                    =b_range, verbose=verbose, device=self.device)
                self.funs[j][i] = fun
                self.funs_avoid_singularity[j][i] = fun_avoid_singularity
                self.affine.detach()[j][i] = params
                return r2
        else:
            fun = fun_name
            fun_sympy = fun_name
    ...
    def swap(self, i1, i2, mode='in'):
        with paddle.no_grad():
            ...
            swap_(self.affine.detach(), i1, i2, mode)
            swap_(self.mask.detach(), i1, i2, mode)
