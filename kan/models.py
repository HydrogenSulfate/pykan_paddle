...
class AutoEncoder(KAN):
    ...
    def __init__(self, width_enc=None, width_dec=None, grid=3, k=3, seed=1,
        enc_type='kan', dec_type='kan'):
        super(AutoEncoder, self).__init__(width=[1, 1])
        if enc_type == 'kan':
            self.encoder = KAN(width=width_enc, grid=grid, k=k, seed=seed,
                auto_save=False, base_fun='identity')
        elif enc_type == 'mlp':
            self.encoder = MLP(width=width_enc, seed=seed)
        if dec_type == 'kan':
            self.decoder = KAN(width=width_dec, grid=grid, k=k, seed=seed,
                auto_save=False, base_fun='identity')
        elif dec_type == 'mlp':
    ...
    def get_reg(self, reg_metric='edge_backward', lamb_l1=1.0, lamb_entropy
        =2.0, lamb_coef=1.0, lamb_coefdiff=0.0):
        if self.enc_type == 'kan':
            enc_reg = self.encoder.reg(reg_metric, lamb_l1, lamb_entropy,
                lamb_coef, lamb_coefdiff)
        else:
            enc_reg = self.encoder.reg(reg_metric='w', lamb_l1=lamb_l1,
                lamb_entropy=lamb_entropy)
        if self.dec_type == 'kan':
            dec_reg = self.decoder.reg(reg_metric, lamb_l1, lamb_entropy,
                lamb_coef, lamb_coefdiff)
        else:
            dec_reg = self.decoder.reg(reg_metric='w', lamb_l1=lamb_l1,
                lamb_entropy=lamb_entropy)
    ...
    def fit(self, dataset, reg_metric='edge_backward', steps=20, lamb=0.0,
        lamb_l1=1.0, lamb_entropy=2.0, lamb_coef=1.0, lamb_coefdiff=0.0):
        super(AutoEncoder, self).fit(dataset, reg_metric=reg_metric, steps=
            steps, lamb=lamb, lamb_l1=lamb_l1, lamb_entropy=lamb_entropy,
