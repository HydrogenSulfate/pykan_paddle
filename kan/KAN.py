...
class KAN(paddle.nn.Layer):
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1,
        scale_base_mu=0.0, scale_base_sigma=1.0, base_fun=paddle.nn.Silu(),
        symbolic_enabled=True, bias_trainable=False, grid_eps=1.0,
        grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, device=
        'cpu', seed=0):
        """
        ...
        for l in range(self.depth):
            scale_base = scale_base_mu * 1 / np.sqrt(width[l]
                ) + scale_base_sigma * (paddle.randn(shape=[width[l], width
                [l + 1]]) * 2 - 1) * 1 / np.sqrt(width[l])
            sp_batch = KANLayer(in_dim=width[l], out_dim=width[l + 1], num=
                grid, k=k, noise_scale=noise_scale, scale_base=scale_base,
                scale_sp=1.0, base_fun=base_fun, grid_eps=grid_eps,
                grid_range=grid_range, sp_trainable=sp_trainable,
                sb_trainable=sb_trainable, device=device)
            self.act_fun.append(sp_batch)
            out_9 = paddle.nn.Linear(in_features=width[l + 1], out_features
                =1, bias_attr=False)
            out_9.stop_gradient = not bias_trainable
            bias = out_9
            bias.weight.detach() *= 0.0
            self.biases.append(bias)
        self.biases = paddle.nn.LayerList(sublayers=self.biases)
        ...
        self.symbolic_fun = []
        for l in range(self.depth):
            sb_batch = Symbolic_KANLayer(in_dim=width[l], out_dim=width[l +
                1], device=device)
    ...
    def initialize_from_another_model(self, another_model, x):
        ...
        tensor(0.0506)
        """
        another_model(x.to(another_model.place))
        batch = tuple(x.shape)[0]
        self.initialize_grid_from_another_model(another_model, x.to(
            another_model.place))
        for l in range(self.depth):
            spb = self.act_fun[l]
            ...
            self.act_fun[l].coef.data = curve2coef(preacts.reshape(batch,
                spb.size).transpose(perm=[1, 0]), postsplines.reshape(batch,
                spb.size).transpose(perm=[1, 0]), spb.grid, k=spb.k, device
                =self.device)
        ...
        for l in range(self.depth):
            self.biases[l].weight.data = another_model.biases[l
                ].weight.detach()
        for l in range(self.depth):
    ...
    def plot(self, folder='./figures', beta=3, mask=False, mode=
        'supervised', scale=0.5, tick=False, sample=False, in_vars=None,
        out_vars=None, title=None):
        """
        plot KAN
        ...
        for l in range(depth):
            ...
            for i in range(self.width[l]):
                for j in range(self.width[l + 1]):
                    ...
                    numerical_mask = self.act_fun[l].mask.reshape(self.
                        width[l + 1], self.width[l])[j][i]
                    ...
                    lock_id = self.act_fun[l].lock_id[j * self.width[l] + i
                        ].astype(dtype='int64').item()
                    if lock_id > 0:
        ...
        alpha = [score2alpha(score.cpu().detach().numpy()) for score in
            self.acts_scale]
        ...
        for l in range(neuron_depth):
            ...
            spacing = A / n
            for i in range(n):
                plt.scatter(1 / (2 * n) + i / n, l * y0, s=min_spacing ** 2 *
                    10000 * scale ** 2, color='black')
                if l < neuron_depth - 1:
                    n_next = width[l + 1]
                    N = n * n_next
                    for j in range(n_next):
                        ...
                        if mask == True:
                            plt.plot([1 / (2 * n) + i / n, 1 / (2 * N) + 
                                id_ / N], [l * y0, (l + 1 / 2) * y0 - y1],
                                color=color, lw=2 * scale, alpha=alpha[l][j
                                ][i] * self.mask[l][i].item() * self.mask[l +
                                1][j].item())
                            plt.plot([1 / (2 * N) + id_ / N, 1 / (2 *
                                n_next) + j / n_next], [(l + 1 / 2) * y0 +
                                y1, (l + 1) * y0], color=color, lw=2 *
                                scale, alpha=alpha[l][j][i] * self.mask[l][
                            ...
                            plt.plot([1 / (2 * N) + id_ / N, 1 / (2 *
                                n_next) + j / n_next], [(l + 1 / 2) * y0 +
                                y1, (l + 1) * y0], color=color, lw=2 *
                                scale, alpha=alpha[l][j][i] * alpha_mask)
            plt.xlim(0, 1)
        ...
        if in_vars != None:
            ...
            for i in range(n):
                plt.gcf().get_axes()[0].text(1 / (2 * n) + i / n, -0.1,
                    in_vars[i], fontsize=40 * scale, horizontalalignment=
                    'center', verticalalignment='center')
        if out_vars != None:
            n = self.width[-1]
            for i in range(n):
                plt.gcf().get_axes()[0].text(1 / (2 * n) + i / n, y0 * (len
                    (self.width) - 1) + 0.1, out_vars[i], fontsize=40 *
                    scale, horizontalalignment='center', verticalalignment=
                    'center')
        if title != None:
            plt.gcf().get_axes()[0].text(0.5, y0 * (len(self.width) - 1) + 
                0.2, title, fontsize=40 * scale, horizontalalignment=
                'center', verticalalignment='center')

    def train(self, dataset, opt='LBFGS', steps=100, log=1, lamb=0.0,
        lamb_l1=1.0, lamb_entropy=0.0, lamb_coef=0.0, lamb_coefdiff=0.0,
        update_grid=True, grid_update_num=10, loss_fn=None, lr=1.0,
        stop_grid_update_step=50, batch=-1, small_mag_threshold=1e-16,
        small_reg_factor=1.0, metrics=None, sglr_avoid=False, save_fig=
        False, in_vars=None, out_vars=None, beta=3, save_fig_freq=1,
        img_folder='./video'):
        """
        training
        ...
        def reg(acts_scale):
            ...
            def nonlinear(x, th=small_mag_threshold, factor=small_reg_factor):
                return (x < th) * x * factor + (x > th) * (x + (factor - 1) *
                    th)
            reg_ = 0.0
            for i in range(len(acts_scale)):
                vec = acts_scale[i].reshape(-1)
                p = vec / paddle.sum(x=vec)
                l1 = paddle.sum(x=nonlinear(vec))
                entropy = -paddle.sum(x=p * paddle.log2(x=p + 0.0001))
                reg_ += lamb_l1 * l1 + lamb_entropy * entropy
            for i in range(len(self.act_fun)):
                coeff_l1 = paddle.sum(x=paddle.mean(x=paddle.abs(x=self.
                    act_fun[i].coef), axis=1))
                coeff_diff_l1 = paddle.sum(x=paddle.mean(x=paddle.abs(x=
                    paddle.diff(x=self.act_fun[i].coef)), axis=1))
        ...
        def closure():
            ...
            pred = self.forward(dataset['train_input'][train_id].to(self.
                device))
            if sglr_avoid == True:
                id_ = paddle.nonzero(paddle.isnan(x=paddle.sum(x=pred, axis=1)
                    ) == False, as_tuple=True)[0]
                train_loss = loss_fn(pred[id_], dataset['train_label'][
                    train_id][id_].to(self.device))
            else:
        ...
        for _ in pbar:
            ...
            if (_ % grid_update_freq == 0 and _ < stop_grid_update_step and
                update_grid):
                self.update_grid_from_samples(dataset['train_input'][
            ...
            if opt == 'Adam':
                pred = self.forward(dataset['train_input'][train_id].to(
                    self.device))
                if sglr_avoid == True:
                    id_ = paddle.nonzero(paddle.isnan(x=paddle.sum(x=pred,
                        axis=1)) == False, as_tuple=True)[0]
                    train_loss = loss_fn(pred[id_], dataset['train_label'][
                        train_id][id_].to(self.device))
                else:
                ...
                loss.backward()
                optimizer.step()
            test_loss = loss_fn_eval(self.forward(dataset['test_input'].to(
                self.device)), dataset['test_label'].to(self.device))
            if _ % log == 0:
                pbar.set_description(
                    'train loss: %.2e | test loss: %.2e | reg: %.2e ' % (
                    paddle.sqrt(x=train_loss).cpu().detach().numpy(),
                    paddle.sqrt(x=test_loss).cpu().detach().numpy(), reg_.
                    cpu().detach().numpy()))
            if metrics != None:
                for i in range(len(metrics)):
    ...
    def prune(self, threshold=0.01, mode='auto', active_neurons_id=None):
        """
        ...
        """
        mask = [paddle.ones(shape=self.width[0])]
        active_neurons = [list(range(self.width[0]))]
        for i in range(len(self.acts_scale) - 1):
            if mode == 'auto':
                in_important = (paddle.max(x=self.acts_scale[i], axis=1),
                    paddle.argmax(x=self.acts_scale[i], axis=1))[0] > threshold
                out_important = (paddle.max(x=self.acts_scale[i + 1], axis=
                    0), paddle.argmax(x=self.acts_scale[i + 1], axis=0))[0
                    ] > threshold
                overall_important = in_important * out_important
            elif mode == 'manual':
                overall_important = paddle.zeros(shape=self.width[i + 1],
                    dtype='bool')
                overall_important[active_neurons_id[i + 1]] = True
            mask.append(overall_important.astype(dtype='float32'))
            active_neurons.append(paddle.nonzero(overall_important == True, as_tuple=True)[0])
        active_neurons.append(list(range(self.width[-1])))
        mask.append(paddle.ones(shape=self.width[-1]))
        self.mask = mask
        for l in range(len(self.acts_scale) - 1):
            for i in range(self.width[l + 1]):
                if i not in active_neurons[l + 1]:
                    self.remove_node(l + 1, i)
        model2 = KAN(copy.deepcopy(self.width), self.grid, self.k, base_fun
            =self.base_fun, device=self.device)
        model2.set_state_dict(state_dict=self.state_dict())
        for i in range(len(self.acts_scale)):
            if i < len(self.acts_scale) - 1:
                model2.biases[i].weight.data = model2.biases[i
                    ].weight.detach()[:, active_neurons[i + 1]]
            model2.act_fun[i] = model2.act_fun[i].get_subset(active_neurons
                [i], active_neurons[i + 1])
    ...
    def remove_node(self, l, i):
        ...
        --------
            None
        """
        self.act_fun[l - 1].mask[i * self.width[l - 1] + paddle.arange(end=
            self.width[l - 1])] = 0.0
        self.act_fun[l].mask[paddle.arange(end=self.width[l + 1]) * self.
            width[l] + i] = 0.0
            ...
            for i in range(self.width[l]):
                for j in range(self.width[l + 1]):
                    if self.symbolic_fun[l].mask[j, i] > 0.0:
                        ...
                        name, fun, r2 = self.suggest_symbolic(l, i, j,
    ...
    def symbolic_formula(self, floating_digit=2, var=None, normalizer=None,
        simplify=False, output_normalizer=None):
        """
            ...
            for j in range(self.width[l + 1]):
                ...
                for i in range(self.width[l]):
                    ...
                    try:
                        ...
                        return
                if simplify == True:
                    y.append(sympy.simplify(yj + self.biases[l].weight.
                        detach()[0, j]))
                else:
                    y.append(yj + self.biases[l].weight.detach()[0, j])
            x = y
            symbolic_acts.append(x)
        ...
        self.symbolic_acts = [[ex_round(symbolic_acts[l][i]) for i in range
            (len(symbolic_acts[l]))] for l in range(len(symbolic_acts))]
        out_dim = len(symbolic_acts[-1])
        return [ex_round(symbolic_acts[-1][i]) for i in range(len(
            symbolic_acts[-1]))], x0
    ...
    def save_ckpt(self, name, folder='./model_ckpt'):
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        paddle.save(obj=self.state_dict(), path=folder + '/' + name)
    ...
    def load_ckpt(self, name, folder='./model_ckpt'):
        ...
        --------
            None
        """
        self.set_state_dict(state_dict=paddle.load(path=str(folder + '/' +
            name)))
